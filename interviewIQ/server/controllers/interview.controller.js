...
`
        ...
        try {
            ...
            console.error("Resume AI parse failed. Raw response:", aiResponse);
            await fs.promises.unlink(filepath).catch(() => {});
            return res.status(502).json({
                message: "AI failed to analyze the resume. Please try again.",
            });
        }
...
`
        ...
        try {
            ...
            console.error("Answer AI parse failed. Raw response:", aiResponse);
            return res.status(502).json({
                message: "Failed to evaluate the answer. Please try again.",
            });
        }
...
export const getMyInterviews = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            message: `failed to find currentUser Interviews ${error}`
        });
    }
};
...
export const getInterviewReport = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            message: `Failed to get interview report ${error}`
        });
    }
};